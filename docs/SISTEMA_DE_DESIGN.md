# Sistema de design do Eclesiastes

## Direção

O portal replica a linguagem funcional do AVA oficial apenas no fluxo de disciplinas e aulas. A base visual é clara, compacta e orientada por navegação: barra azul-petróleo, fundo cinza muito claro, superfícies brancas, bordas discretas e cartões com raio moderado. Inter é usada nos títulos, subtítulos, controles e elementos de navegação; Nunito é usada na leitura de conteúdo; os ícones são locais do Font Awesome.

## Componentes

- Catálogo: abertura “Página inicial” identificada como repositório de respostas e cartões de disciplina em duas colunas no desktop e uma coluna no celular. Cada disciplina recebe ícone e cor próprios, sem estado fictício de favorito.
- Disciplina: cabeçalho próprio azul-petróleo no topo do conteúdo, guia de navegação sem backend, totais reais e bimestres/semanas reunidos em painéis estruturados. As rotas antigas de bimestre redirecionam para essa visão unificada.
- Semana: painel branco com trilha, grupos expansíveis de aula, setas estáveis e recursos tipados com bloco visual para PDF, H5P e registro.
- Recurso: cabeçalho do item, cartão de conteúdo e o aviso único de indisponibilidade.
- Sumário: lateral contextual com semanas, itens da semana atual, estados ativos e opções para expandir ou retrair tudo. Pode ser fechado por botão, arrasto horizontal com mouse ou Escape; o controle flutuante fixo à esquerda o reabre. No celular inicia fechado.

## Acessibilidade e tema

Há link para pular ao conteúdo, foco perceptível, marcos semânticos, controles nomeados e redução de movimento. O tema claro segue a referência visual; o tema escuro é uma adaptação do próprio portal e é alternado por ícones de lua e sol.
